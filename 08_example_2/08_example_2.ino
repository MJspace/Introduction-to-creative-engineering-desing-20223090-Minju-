...
// turn on the LED if the distance is between dist_min and dist_max  
  if (100<=dist_raw && dist_raw<=200) {
    analogWrite(PIN_LED, (200-dist_raw)/100*255);
  }
  if (dist_raw>200 && dist_raw<=300) {
    analogWrite(PIN_LED, (dist_raw-200)/100*255);
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if (reading != 0) last_raw = reading;
  if (reading == 0) reading = last_raw;
  return reading;
